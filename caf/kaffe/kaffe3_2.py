...
def do_it3(layer,net,iter_n,start=0):
    ...
    layer_shape[0] = 1
    layer_shape = tuple(layer_shape)
    img_path = opj(home_path,'scratch/2015/10/27/'+model_folders[MODEL_NUM]+'/'+layer.replace('/','-'))
    unix('mkdir -p ' + img_path)
    for n in [start]:#range(start,layer_shape[1]):#(num_nodes):
        ...
        for i in range(iter_n):
            ...
            make_step(net,jitter=32,end=layer,objective=objective_kz7,clip=True)
            src = net.blobs['data']
            #vis = deprocess(net, src.data[0])
            if np.mod(i,10.0)==0:
                pb.animate(i+1)
            ...
            if np.mod(i,f)==0:
                ...
                img = np.uint8(np.clip(vis, 0, 255))
                #mi(img,opj(img_path,str(n)+'.png'))
                imsave(opj(img_path,str(n)+'.random_dot_jitter_32'+tm+'.'+str(i)+'.png'),img)
...
if True:
    ...
    for r in range(100):
        for l in ['inception_4d/5x5']:#['inception_4e/output']:#['fc8']:
            do_it3(l,net,50000,24)
