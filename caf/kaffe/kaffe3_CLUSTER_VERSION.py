...
def do_it3(layer,net,iter_n,start=0):
    ...
    layer_shape[0] = 1
    layer_shape = tuple(layer_shape)
    img_path = opj(home_path,'scratch/2015/10/22/'+model_folders[MODEL_NUM]+'/'+layer.replace('/','-'))
    unix('mkdir -p ' + img_path)
    for n in range(start,layer_shape[1]):#(num_nodes):
        mask7 = np.zeros(layer_shape)
...
if True:
    ...
    for l in ['loss3/classifier']: #['pool5/7x7_s1']: #['conv1/7x7_s2','conv2/3x3']: #inception_layers: #['inception_4b/5x5','inception_4b/3x3','inception_4b/1x1','inception_4a/5x5','inception_4a/3x3','inception_4a/1x1']:
        do_it3(l,net,4000,0)
