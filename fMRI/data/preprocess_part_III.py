...
is not identical for each run. Thus, consensus masks are created. This is coordinated by:

	create_and_save_session_consensus_mask_wrapper(work_path,to_do)

which saves the results in consensus_mask.nii.gz in the subjects directory for that session. I am not sure
...
Average volumes combine data from separate runs. The function:

	get_run_averages_and_r_volume_wrapper(work_path,to_do_func_runs,to_do_stats)

saves the following in the relevant experiments folder:
...
10 Nov. 2015. One thing to note is that the usage of the 'to_do' list keeps changing from function to function, both here
and in the previous pre-processing module.
...
# 11/10/2015
work_path = opjD('')
to_do_func_runs = ['S1_2015/2015/7/10/0/func_runs/pp_a0']
to_do_stats = ['S1_2015/2015/7/10/0/stats/pp_a0']
get_run_averages_and_r_volume_wrapper(work_path,to_do_func_runs,to_do_stats)
...
def get_run_averages_and_r_volume_wrapper(work_path,to_do_func_runs,to_do_stats):
	"""
	e.g.,
	work_path = opjD('')
	to_do_func_runs = ['S1_2015/2015/7/10/0/func_runs/pp_a0']
	...
	for s in subject_stats_paths:
		load_stats_dir(s,All_subject_stats_Dic)
	experiment_func_runs_paths.sort(key=natural_keys)
	for f in experiment_func_runs_paths:
		Func_runs_Dic = {}
		session_str = extract_session_from_func_run_path(f)
		cm = All_subject_stats_Dic[select_keys(All_subject_stats_Dic,[session_str,'consensus_mask.nii.gz:data'])[0]]
		load_func_runs(f,Func_runs_Dic,['filtered_func_data.nii.gz','mean_func.nii.gz','mask.nii.gz'])
		...
		with open(opj(os.path.expanduser("~"),'Desktop/correlation_errors.txt'), "a") as myfile:
							myfile.write('\n'+f+'\n')
		get_run_averages_and_r_volume(cm,Func_runs_Dic,'','',dst_path)
