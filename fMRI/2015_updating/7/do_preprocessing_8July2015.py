		...
		if Combine_P_Images:
			...
			if sub_experiments[0] == 'Vermeer' or sub_experiments[0] == 'Kay_images':
				...
				for i in range(len(Img_Dic[sub_experiments[0]])):
					mi(img_varient(Img_Dic[sub_experiments[0]][i],scale_factor[0],False),opj(subject,sub_experiments[0],str(month),str(day)),[n_images+1,1+len(conditions),1+i*(1+len(conditions))],'')	
					mi(img_varient(Img_Dic[sub_experiments[0]][i],scale_factor[0],False),opj(subject,sub_experiments[0],str(month),str(day),'YB'),[n_images+1,1+len(conditions),1+i*(1+len(conditions))],'')	
				...
				for task in conditions:
					...
					for i in range(n_images):
						...
						if i == 0:
							title_str = task
						mi(z2o(avg_img)**1.0,opj(subject,sub_experiments[0],str(month),str(day)),[n_images+1,1+len(conditions),1+task_ctr+i*(1+len(conditions))],title_str)
						...
						mi(ci,opj(subject,sub_experiments[0],str(month),str(day),'YB'),[n_images+1,1+len(conditions),1+task_ctr+i*(1+len(conditions))],title_str)
						
			elif sub_experiments[0] == 'Three_circles':
				n_images = 4
				scale_factor = [2.0,1.5,1.0]
				for i in range(len(Img_Dic[sub_experiments[0]])):
					mi(img_varient(Img_Dic[sub_experiments[0]][i],scale_factor[0],False),'differences: ' + opj(subject,sub_experiments[0],str(month),str(day)),[n_images+1,1+len(conditions),1+i*(1+len(conditions))],title_str)	
					mi(img_varient(Img_Dic[sub_experiments[0]][i],scale_factor[0],False),'differences: ' + opj(subject,sub_experiments[0],str(month),str(day),'YB'),[n_images+1,1+len(conditions),1+i*(1+len(conditions))],title_str)	
				...
				for task in conditions:
					...
					for i in range(n_images):
						...
						for k in range(len(scale_factor)):
							...
							avg_img += img1
							ctr += 1
							mi(img1,opj(subject,sub_experiments[0],str(month),str(day),'separate'),[4,6,ctr],str(ctr))
							mn = avg_img.mean()
							for x in range(np.shape(avg_img)[0]):
								for y in range(np.shape(avg_img)[1]):
									if np.sqrt((x-np.shape(avg_img)[0]/2)**2+(y-np.shape(avg_img)[1]/2)**2) < fix_mask_radius:
						...
						if i == 0:
							title_str = task+' '
						mi(z2o(avg_img)**1.0,opj(subject,sub_experiments[0],str(month),str(day)),[n_images+1,1+len(conditions),1+i*(1+len(conditions))],title_str + str(i))
						avgs.append(avg_img)
				...
				for i in range(n_images):
					...
					mi(ci,'differences: ' + opj(subject,sub_experiments[0],str(month),str(day),'YB'),[n_images+1,1+len(conditions),1+task_ctr+i*(1+len(conditions))],title_str + str(i))
					mi(z2o(avgs[i]-avg)**1.0,'differences: ' + opj(subject,sub_experiments[0],str(month),str(day)),[n_images+1,1+len(conditions),1+task_ctr+i*(1+len(conditions))],title_str + str(i))
				...
				n_images = 8
				for i in range(len(Img_Dic[sub_experiments[0]])):
					mi(img_varient(Img_Dic[sub_experiments[0]][i],scale_factor[0],False)[130:637,308:715],opj(subject,sub_experiments[0],str(month),str(day)),[n_images+1,1+len(conditions),1+i*(1+len(conditions))],'')	
					mi(img_varient(Img_Dic[sub_experiments[0]][i],scale_factor[0],False)[130:637,308:715],opj(subject,sub_experiments[0],str(month),str(day),'YB'),[n_images+1,1+len(conditions),1+i*(1+len(conditions))],'')	
				...
				for task in conditions:
					...
					for i in range(n_images):
						...
						mi(z2o(avg_img_big)**1.0,opj(subject,sub_experiments[0],str(month),str(day)),[n_images+1,1+len(conditions),1+task_ctr+i*(1+len(conditions))],title_str)
						...
						mi(ci,opj(subject,sub_experiments[0],str(month),str(day),'YB'),[n_images+1,1+len(conditions),1+task_ctr+i*(1+len(conditions))],title_str)
				for i in range(len(Img_Dic[sub_experiments[0]])):
					mi(SES[ses]['Tasks']['attend_face'][i]-SES[ses]['Tasks']['attend_place'][i],opj(subject,sub_experiments[0],str(month),str(day),'DIFF'),[n_images+1,1+len(conditions),1+1+i*(1+len(conditions))],title_str)
					mi(-SES[ses]['Tasks']['attend_face'][i]+SES[ses]['Tasks']['attend_place'][i],opj(subject,sub_experiments[0],str(month),str(day),'DIFF'),[n_images+1,1+len(conditions),1+2+i*(1+len(conditions))],title_str)
					stim = (z2o(img_varient(Img_Dic[sub_experiments[0]][i],scale_factor[0],False)[:,:,0])-0.5)[130:637,308:715]
					ci = stim * z2o(SES[ses]['Tasks']['attend_face'][i]-SES[ses]['Tasks']['attend_place'][i])**2
					mi(ci,opj(subject,sub_experiments[0],str(month),str(day),'DIFF MOD'),[n_images+1,1+len(conditions),1+1+i*(1+len(conditions))],title_str)
					ci = stim * z2o(-SES[ses]['Tasks']['attend_face'][i]+SES[ses]['Tasks']['attend_place'][i])**2
					mi(ci,opj(subject,sub_experiments[0],str(month),str(day),'DIFF MOD'),[n_images+1,1+len(conditions),1+2+i*(1+len(conditions))],title_str)
