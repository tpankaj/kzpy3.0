...
def pwm_to_percent(in_pwm,min_pwm,pwm_null,max_pwm):
    if in_pwm >= pwm_null:
        out_percent = int((in_pwm - pwm_null)/(1.0*max_pwm - 1.0*pwm_null)*50 + 49)
    else:
        out_percent = int((in_pwm - pwm_null)/(1.0*pwm_null - 1.0*min_pwm)*49+49)
...
while True:
    ...
    try:
        ...
        in_motor_pwm = t[0]
        in_button_pwm = t[2]
        out_steer = pwm_to_percent(in_steer_pwm,servo_min_cpu,servo_null,servo_max_cpu)
        out_motor = pwm_to_percent(in_motor_pwm,motor_min_cpu,motor_null,motor_max_cpu)
