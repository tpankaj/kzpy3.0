...
##############################################################################
#
model_path = opjh("kzpy3/caf6/z2_color_gradient2")
version = 'version 1b'
gpu = 0
base_lr = 0.005
snapshot = 100000
...
train_val_lst = [
	...
	dummy('conv1_gradient',(1,2,28,53)),
	dummy('conv2_gradient',(1,2,6,12)),

	conv("ninGa",'ZED_data_pool2_gradient',20,1,1,1,0,"gaussian",std='0.1'),
	relu("ninGa"),
	conv("ninGb",'ninGa',5,1,1,1,0,"gaussian",std='0.1'),
	relu("ninGb"),

	conv("ninGc",'conv1_gradient',40,1,1,1,0,"gaussian",std='0.1'),
	relu("ninGc"),
	conv("ninGd",'ninGc',20,1,1,1,0,"gaussian",std='0.1'),
	relu("ninGd"),

	conv("ninGe",'conv2_gradient',100,1,1,1,0,"gaussian",std='0.1'),
	relu("ninGe"),
	conv("ninGf",'ninGe',40,1,1,1,0,"gaussian",std='0.1'),
	relu("ninGf"),

	concat('ZED_data_pool2_concat',["ZED_data_pool2","ninGb"],1),

	conv("conv1",'ZED_data_pool2_concat',96,1,11,3,0,"gaussian",std='0.00001'),
	relu("conv1"),
	pool("conv1","MAX",3,1,(1,1)),
	concat('conv1_concat',["conv1","conv1_pool","ninGd"],1),
	...
	concat('conv2_concat',["conv2","conv2_pool","ninGf"],1),
...
if TRAIN:
	...
	gradients = [['ZED_data_pool2_gradient',94,168],['conv1_gradient',28,53],['conv2_gradient',6,12]]
	for g in gradients:
		for y in range(g[1]):
			for x in range(g[2]):
				caffe_net.solver.net.blobs[g[0]].data[0,0,y,x] = y
				caffe_net.solver.net.blobs[g[0]].data[0,1,y,x] = x
		for i in range(2):
			caffe_net.solver.net.blobs[g[0]].data[0,i,:,:] = z2o(caffe_net.solver.net.blobs[g[0]].data[0,i,:,:])-0.5
