...
if True:
	...
	solver_name = opjh('kzpy3/caf7/z2_color/solver_3_frames.prototxt')
	...
	if 'solver_state_1_no_Smyth_or_racing' in solver_name:
		...
		use_states = [1]
	if 'solver_10_frames' in solver_name:
	...
	while True:
		...
		solver.net.blobs['steer_motor_target_data'].data[0,:30] = data['steer'][-30:]/99.
		solver.net.blobs['steer_motor_target_data'].data[0,30:] = data['motor'][-30:]/99.
		past_steer = data['steer'][:10]/99.
		past_motor = data['motor'][:10]/99.
		for i in range(len(past_steer)):
			solver.net.blobs['metadata'].data[0,6+i,:,:] = past_steer[i]
		for i in range(len(past_motor)):
			solver.net.blobs['metadata'].data[0,10+6+i,:,:] = past_motor[i]
		#
		##########################################################
