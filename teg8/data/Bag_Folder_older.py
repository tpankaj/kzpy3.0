...
def init(bag_folders_path_meta_path,bag_folders_rgb_1to4_path, left_image_bound_to_data_name='left_image_bound_to_data.pkl',NUM_STATE_ONE_STEPS=10,accepted_states=[1]):
    ...
    run_name = fname(BF['path'])
    cprint('Bag_Folder::__init__, run = '+run_name,'yellow','on_red')

    BF['bag_files'] = sgg(opj(BF['path'],'*.bag.pkl'))
    cprint(opj(BF['path'],'*.bag.pkl'))
    ...
    good_timestamps_set = set(BF['data']['raw_timestamp_deltas']) # & set(BF['img_dic']['left'].keys()) # Note, won't have this info here now.
    bad_timestamps_list = []
    ...
    cprint('basic checking 1 done','yellow')
    ...
    cprint('basic checking 2 done','yellow')
    ...
    for ts in good_timestamps_list:
        BF['left_image_bound_to_data'][ts]['state_one_steps'] = None # overwrite loaded values
