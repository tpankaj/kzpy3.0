...
def get_data_thread(BagFolder_dic,bag_img_dic,data_queue,group_binned_timestamps,NUM_STATE_ONE_STEPS):
	timer = Timer(60)	
	while True:
		if timer.check():
			timer.reset()
...
#def load_data_package(num_bag_files_to_load):
data_queue = Queue()
...
threading.Thread(target=get_data_thread,args=(BagFolder_dic,bag_img_dic,data_queue,group_binned_timestamps,NUM_STATE_ONE_STEPS)).start()
...
while True:
	...
	if solver_ready_queue.qsize() > 0:
		s = solver_ready_queue.get()
		caffe_net.train_step(s)
		solver_waiting_queue.put(s)
		ctr += 1
"""
while time.time()-t2 < 1.0*load_time: #not ((count_median > 30) and (time.time()-t2 > 5.0*load_time)):
	...
	data_loader_thread_unthreaded(BagFolder_dic,bag_img_dic,data_queue,group_binned_timestamps,NUM_STATE_ONE_STEPS)

	load_data_into_model_thread_unthreaded(version,data_queue,solver_ready_queue,solver_waiting_queue)
