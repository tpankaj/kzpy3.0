...
def data_loader_thread_unthreaded(BagFolder_dic,bag_img_dic,data_queue,group_binned_timestamps,NUM_STATE_ONE_STEPS):
	#print "data_loader_thread"
	while True:
		if data_queue.qsize() < 10 and len(bag_img_dic) > 10:
			data = get_data(BagFolder_dic,bag_img_dic,group_binned_timestamps,NUM_STATE_ONE_STEPS)
...
def load_data_into_model_thread_unthreaded(version,data_queue,solver_ready_queue,solver_waiting_queue):
	while True:
		try:
			if solver_ready_queue.qsize() < 150:
				#print "load_data_into_model_thread"
