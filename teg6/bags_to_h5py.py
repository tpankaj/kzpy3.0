...
if True:
	...
	BN = False #'follow_28Aug2016_Mr_Orange_Tilden/bair_car_2016-08-28-10-36-11_71.bag.pkl'

	CONTINUE = True
	for bn in bag_names_list:
		#if (not CONTINUE) and (BN != bn):
		#	continue
		#CONTINUE = True
		bag_img_dic = {}
			...
			if run_name in BagFolder_dic:
				if 'good_bag_timestamps' in BagFolder_dic[run_name]:
					if bf in BagFolder_dic[run_name]['good_bag_timestamps']:
						...
						timestamps = binned_timestamps[0][:ln]+binned_timestamps[1][:ln]
						assert(len(timestamps) == 2*ln)
							...
							for ts in timestamps:
								...
								if data != None:
									...
									if timer.check(): #mod(ctr,30)==0:#
										...
										cprint(d2s('ctr =',ctr,'rate =',dp(ctr/(time.time()-t0),1),'Hz','size =',dp(os.path.getsize(hdf5_filename)/10**12.,4),'TB'),'red','on_yellow') #ctr = 0
