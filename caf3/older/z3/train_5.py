...
weights_file_path4 = opjD('z3/z3_5_iter_75000.caffemodel') #None #opjD('z3/z3_4_iter_5000.caffemodel')
...
while True:
	try:
		...
		for i in range(len(solver4_list)):
			...
			solver4_list[i].step(1)



		a = solver4_list[-1].net.blobs['steer_motor_target_data'].data[0,:] - solver4_list[-1].net.blobs['ip2'].data[0,:]
		...
		if datashow:
			...
			print array_to_int_list(solver4_list[-1].net.blobs['steer_motor_target_data'].data[0,:][:])
			print array_to_int_list(solver4_list[-1].net.blobs['ip2'].data[0,:][:])
