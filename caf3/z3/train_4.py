...
weights_file_path3 = opjD('z3/z3_4_iter_5000.caffemodel')
...
while True:
	...
	for i in range(len(solver3_list)):
		...
		solver3_list[i].step(1)


	a = solver3_list[-1].net.blobs['steer_motor_target_data'].data[0,:] - solver3_list[-1].net.blobs['ip2'].data[0,:]
	loss.append(np.sqrt(a * a).mean())
	imshow = False
	datashow = False
	if np.mod(ctr,10) == 0:
	...
	if datashow:
		...
		print array_to_int_list(solver3_list[-1].net.blobs['steer_motor_target_data'].data[0,:][:])
		print array_to_int_list(solver3_list[-1].net.blobs['ip2'].data[0,:][:])
