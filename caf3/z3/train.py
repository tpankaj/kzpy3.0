...
weights_file_path = opjD('z3/z3_iter_440000.caffemodel') #
#
########################################################
...
def load_data_into_model(solver,data):
	...
	if 'left' in data:
		if type(data['left'][0]) == np.ndarray:
			...
			solver.net.blobs['metadata'].data[0,6,:,:] = Caf


			#print solver.net.blobs['metadata'].data[0,:,5,5]
			#time.sleep(0.01)

			steer0 = target_data[0]
			motor0 = target_data[len(target_data)/2]
			for i in range(0,len(target_data)/2):
				solver.net.blobs['steer_motor_target_data'].data[0,i] = (target_data[i]-steer0+49)/99.
			for i in range(len(target_data)/2,len(target_data)):
				solver.net.blobs['steer_motor_target_data'].data[0,i] = (target_data[i]-motor0+49)/99.
