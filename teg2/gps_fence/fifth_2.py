...
def get_pts2(v1,w,graphics=False):
	...
	if graphics:
		...
		plt.xlim(-2,2);plt.ylim(-2,2)
		plt_square()
	return [[v_left[0],-v_bottom[0]],[v_right[0],-v_bottom[0]],[v_left[0],object_screen_width-v_bottom[0]],[v_right[0],object_screen_width-v_bottom[0]]],m
			...
			if y > 0:
				if np.rad2deg(angle_between([1,0],[x,y])) > 35:
					if np.rad2deg(angle_between([1,0],[x,y])) < 180-35:
						plot(x,y,'ro')
						img_pts,m = get_pts3([x,y],0.05,False)
						mi_picture(img_pts,m,False)
		img[100,0]=0.33
		mi(img[100:,0:250],'img')#mi(img[:,50:150],'img')
		plt.pause(0.001)
