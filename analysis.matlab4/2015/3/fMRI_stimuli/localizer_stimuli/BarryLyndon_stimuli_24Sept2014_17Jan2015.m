...
% 17 Jan. convert frames to video
if 1
    outputVideo = VideoWriter(d2s({'~/Desktop/BarryLyndon_redspot_17Jan2015_1.avi'}),'Motion JPEG AVI');%'Uncompressed AVI');%
    outputVideo.FrameRate = 25;
    open(outputVideo);
    h = waitbar(0,'Now processing'); movegui(h,'southeast');
    for i=1:7500
        waitbar(i/7500);
    	fimg = imread(d2s({'~/Desktop/Data/fmri_scanning_experiments/2015/1/17Jan2015_BarryLyndon_material_for_scanning/BarryLyndon_16Jan2015_1.avi_images/' i '.png'}));                ctr = ctr + 1;
        writeVideo(outputVideo,fimg);
    end
	close(outputVideo);
    close(h);
