    ...
    for theta = 0:(360/36):359
        ...
        for x = 1:stimX
            for y = 1:stimY
                ...
                Y = y-stimYcenter;
%                 xysq = X^2+Y^2;
%                 if xysq < (theta/360*min_half_wid*1.4142)^2
%                     mask(ctr,x,y) = 1;
%                     %mask(ctr+24,x,y) = 0;
%                 end
                v=[X Y];
    ...
    for m = 0:(num_masks+1)
        ...
        for i = 1:len
            ...
            if img_num > 0
                ...
                this_mask = 0*this_mask;
            end
            stimulus = merge_images(img,0*frame,this_mask);
            stimulus_BarryLyndon_crop = uint8(255*stimulus(64:447,1:512,:));
            mi(stimulus_BarryLyndon_crop,69,[1,1,1],d2s({i}));
%             letter = 'a';
%             letter(1)=letter(1)+randi(26)-1;
%             text(256,160,the_text(i),'Color','w');
