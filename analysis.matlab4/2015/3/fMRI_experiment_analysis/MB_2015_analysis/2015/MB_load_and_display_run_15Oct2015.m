...
if 1 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if 0
        ...
        ee = 'attend_person_not_BL_BarryLyndon_16Jan2015_2';
        if not(exist(ee))
            eval(d2s({ee ' = MB_get_MB_experiment(''' e ''', ''' ee ''', ''' s ''');'}));
            eval(d2s({ee '_resampled_data = tseriesinterp_selected_voxels(' ee '.runs_avg, RF_mapping.selected_voxel_xyzs, ' ee '.runs(1).TR, 5 );'}));
        end
        ee = 'attend_person_not_BL_BarryLyndon_16Jan2015_3';
        if not(exist(ee))
            eval(d2s({ee ' = MB_get_MB_experiment(''' e ''', ''' ee ''', ''' s ''');'}));
            eval(d2s({ee '_resampled_data = tseriesinterp_selected_voxels(' ee '.runs_avg, RF_mapping.selected_voxel_xyzs, ' ee '.runs(1).TR, 5 );'}));
        end
    ...
    if 1
        ...
        ee = 'attend_scene_BarryLyndon_16Jan2015_2';
        if not(exist(ee))
            eval(d2s({ee ' = MB_get_MB_experiment(''' e ''', ''' ee ''', ''' s ''');'}));
            eval(d2s({ee '_resampled_data = tseriesinterp_selected_voxels(' ee '.runs_avg, RF_mapping.selected_voxel_xyzs, ' ee '.runs(1).TR, 5 );'}));
        end
        ee = 'attend_scene_BarryLyndon_16Jan2015_3';
        if not(exist(ee))
            eval(d2s({ee ' = MB_get_MB_experiment(''' e ''', ''' ee ''', ''' s ''');'}));
            eval(d2s({ee '_resampled_data = tseriesinterp_selected_voxels(' ee '.runs_avg, RF_mapping.selected_voxel_xyzs, ' ee '.runs(1).TR, 5 );'}));
        end
...
if 1
    if not(exist('BL_frames_1'))
        ...
        load('~/Data/stimuli/2015/1/17Jan2015_BarryLyndon_material_for_scanning/BarryLyndon_16Jan2015_1.avi.frames.mat');
        BL_frames_1 = frames;
    end
    if not(exist('BL_frames_2'))
         fps('*** load BL_frames_2');
         load('~/Data/stimuli/2015/1/17Jan2015_BarryLyndon_material_for_scanning/BarryLyndon_16Jan2015_2.avi.frames.mat');
         BL_frames_2 = frames;
    end
    if not(exist('BL_frames_3'))
         fps('*** load BL_frames_3');
         load('~/Data/stimuli/2015/1/17Jan2015_BarryLyndon_material_for_scanning/BarryLyndon_16Jan2015_3.avi.frames.mat');
         BL_frames_3 = frames;
    end
end
...
if 1 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    ci2 = RF_mapping.coverage_img;t=0.04;ci2(ci2<t)=0;ci2(ci2>=t)=1;
    blank = square_single_zeros(512);

    resampled_data = attend_scene_BarryLyndon_16Jan2015_3_resampled_data;%attend_person_not_BL_BarryLyndon_16Jan2015_1_resampled_data;%
    frames = BL_frames_3;


    h = figure(1);
    p_image_frames = zeros(1530,96,128,'single');
    v_frames = zeros(1530,384,512,3,'uint8');
    ...
    for i = 1:(1530)
        ...
        %img(1,1)=1;img(1,2)=-1;
        mi(p_image_frames(i,:,:),1,[1,2,1],d2sp({round(i/5)-6}));%,d2sp({one_TRs(round(1+i/5/0.9)) two_TRs(round(1+i/5/0.9)) round(i/5)}));
        f = squeeze(frames(j,:,:,:));
        fr = imresize(f,[384,512]);
        v_frames(i,:,:,:) = fr;
        mi(v_frames(i,:,:,:),1,[1,2,2],d2s({j/25}));
        F=getframe(h);
        writeVideo(outputVideo,F.cdata);
        drawnow;%pause(1/5);%
    end
    my_save(p_image_frames,'p_image_frames');
    my_save(v_frames,'v_frames');
    close(outputVideo);
end