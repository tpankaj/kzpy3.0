...
if 0
	for i = 1:417
		mi(makeimagestack(sum(z_scored_20150220_082727mbboldmb820mmAPPSNs004a001.z_img(:,:,:,i:(i+3)),4)),99);
		pause(0.1);
	end
end

if 0
	fr_5Hz = video_details.frame_record;
	fr_5Hz(find(fr_5Hz(:) == -1)) = 46;
	fr_5Hz(find(fr_5Hz(:) == -3)) = 47;
	mp({fr_5Hz,'o'},1);
end

 if 0
 	for i = 1:length(fr_5Hz)
 		if fr_5Hz(i)<46
 			img2 = sqsing(v(fr_5Hz(i),:,:,:))/255;
 			mi(img2,1);
 			imwrite(img2,d2s({'~/Desktop/temp3/' fr_5Hz(i) '.png'}),'png');
 			pause(0.1);
 		end
 	end
 end
 ...
 if 0
 	f_5Hz = zeros(1508,1,'int8')+48;
 	f_5Hz(31:(31+1448-1)) = fr_5Hz;
 	mp({f_5Hz,'o'},1);
 end

if 0
	f_5Hz_matrix = zeros(1508,45);
	for i = 1:length(f_5Hz)
		if f_5Hz(i)<46
			f_5Hz_matrix(i,f_5Hz(i)) = 1;
		end
	end
	mi(f_5Hz_matrix,1);
end
...
if 0
	design2={};
	for i = 1:45
		design2{i} = find(f_5Hz_matrix(:,i)>0)/5;
	end
end

 if 0
 	tr_ts = zeros(416,1,'single');
 	tr_t = 0;
 	for i = 1:length(tr_ts)
 		tr_ts(i) = tr_t;
 		tr_t = tr_t + 0.736;
 	end
 	mp({tr_ts,'o'},1);
...
if 0
	f_tr = zeros(length(tr_ts),1,'int16')+49;
 	for i = 1:409%(length(tr_ts)-1)
 		fs = round([tr_ts(i) tr_ts(i+1)]*5)+1;
 		f_tr(i) = int16(min(f_5Hz(fs(1):(fs(2)-1))));
 	end
 	mp({f_tr,'o'},1);
end

if 0
	design1 = zeros(416,49,'single');
 	for i = 1:length(f_tr)
 		design1(i,f_tr(i)) = 1;
 	end
 	mi(design1,1);
...
if 1
	save('~/Desktop/Matlab_Desktop/results.mat','-struct','results', '-v7.3');
	save('~/Desktop/Matlab_Desktop/denoiseddata.mat','denoiseddata', '-v7.3');
end
