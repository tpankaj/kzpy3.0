...
for p=1:length(m)
  ...
  parfor q=1:length(chunks)
    temp{q} = interp1(timeorig,m{p}(:,chunks{q}),timenew,'pchip','extrap');
    %temp{q} = interp1(timeorig,m{p}(:,chunks{q}),timenew,'cubic','extrap');
    %changed 3/8/2015 due to message: Warning: INTERP1(...,'CUBIC') will change in a future release. Use INTERP1(...,'PCHIP') instead.
  end
  m{p} = catcell(2,temp);
  clear temp;
