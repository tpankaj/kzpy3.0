...
def init(path, preprocessed_dir='.preprocessed',left_image_bound_to_data_name='left_image_bound_to_data.pkl',NUM_STATE_ONE_STEPS=10):
    ...
    left_path = opj(path,preprocessed_dir,left_image_bound_to_data_name)
    ...
    for i in range(len(good_timestamps_list)-2,-1,-1):
        if _is_timestamp_valid_data(BF,good_timestamps_list[i]) and good_timestamps_list[i+1] - good_timestamps_list[i] < 0.1:
            state_one_steps += 1
        else:
            state_one_steps = 0
...
def _is_timestamp_valid_data(BF,t):
    valid = True
